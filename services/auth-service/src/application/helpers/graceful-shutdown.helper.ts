...
export class GracefulShutdownHelper extends BaseGracefulShutdownHelper {
  protected static async disconnectServices (): Promise<void> {
    ...
    const disconnectPromises = [
      RetryHelper.executeWithRetry(() => redis.disconnect({ clientId: ClientIds.AUTH_SERVICE }), {
        serviceName: 'Redis',
      ...
      RetryHelper.executeWithRetry(() => db.disconnect({ clientId: ClientIds.AUTH_SERVICE }), {
