...
export class GracefulShutdownHelper extends BaseGracefulShutdownHelper {
  ...
  protected async disconnectServices (): Promise<void> {
    const disconnectPromises = [
      RetryHelper.executeWithRetry(() => this.redis.disconnect({ clientId: ClientIds.ORDER_SERVICE }), {
        serviceName: 'Redis',
      ...
      RetryHelper.executeWithRetry(() => this.db.disconnect({ clientId: ClientIds.ORDER_SERVICE }), {
