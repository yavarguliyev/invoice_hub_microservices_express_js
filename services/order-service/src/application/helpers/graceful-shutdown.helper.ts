...
export class GracefulShutdownHelper extends BaseGracefulShutdownHelper {
  protected static async disconnectServices (): Promise<void> {
    const kafka = new KafkaInfrastructure({ clientId: ClientIds.INVOICE_SERVICE, groupId: GroupIds.ORDER_SERVICE_GROUP });
    const redis = new RedisInfrastructure();
    const db = new DbConnectionInfrastructure();

    const disconnectPromises = [
      RetryHelper.executeWithRetry(() => redis.disconnect({ clientId: ClientIds.ORDER_SERVICE }), {
        serviceName: 'Redis',
      ...
      RetryHelper.executeWithRetry(() => db.disconnect({ clientId: ClientIds.ORDER_SERVICE }), {
